...
describe('Money component', () => {
  test.each(testCases)('%p (%p) -> %p', async (input, value, emitted) => {
    ...
    /*
      ...
      2. По тесту ожидалось, что если value пустое, то не должно быть emit.
      Но в таком случае как очищать поле у родителя (кроме this.$parent)?

      Мне кажется это скорее ошибка в тесте, поэтому изменил проверку
    */
